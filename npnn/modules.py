...
class ELU(Module):
    ...
    def forward(self, x, train=True):
        ...
        """
        self.x = x 
        elu_output = np.where(x > 0, x, self.alpha * (np.exp(x) - 1))
        return elu_output
    ...
    def backward(self, grad):
        ...
        """
        dLdx = np.zeros_like(grad)
        for i in range(grad.shape[0]):  
            for j in range(grad.shape[1]):  
                if self.x[i, j] > 0:
                    dLdx[i, j] = grad[i, j]
                else:
                    dLdx[i, j] = grad[i, j] * self.alpha * np.exp(self.x[i, j])
...
class Dense(Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        W = np.random.uniform(low=-limit, high=limit, size=(dim_out, dim_in))
        b = np.zeros(dim_out)
    ...
    def forward(self, x, train=True):
        ...
        output = np.dot(self.x, W.value.T) + b.value  
        return output
    ...
    def backward(self, grad):
        ...
        b.grad = grad.sum(axis=0) / batch_size
        dx = np.dot(grad, W.value)
        assert np.shape(self.x) == np.shape(dx), "Mismatch in input and output gradient shapes"
        assert np.shape(W.value) == np.shape(W.grad), "Mismatch in weight and weight gradient shapes"
        assert np.shape(b.value) == np.shape(b.grad), "Mismatch in bias and bias gradient shapes"
